...
Write-Host "=== 1/5 smoke-test ===" -ForegroundColor Cyan
& "$PSScriptRoot\smoke-test.ps1" -ApiUrl $ApiUrl
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

if (-not $SkipIsolation) {
    Write-Host ""
    Write-Host "=== 2/5 clinic-isolation-test ===" -ForegroundColor Cyan
    & "$PSScriptRoot\clinic-isolation-test.ps1" -ApiUrl $ApiUrl -SkipSignalR:$SkipSignalR
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
}

Write-Host ""
Write-Host "=== 3/5 routing-test ===" -ForegroundColor Cyan
if ($SkipSlow) {
...
if (-not $SkipEmbed) {
    Write-Host ""
    Write-Host "=== 4/5 embed-session-test ===" -ForegroundColor Cyan
    & "$PSScriptRoot\embed-session-test.ps1" -ApiUrl $ApiUrl
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

    Write-Host ""
    Write-Host "=== 5/5 embed-isolation-test ===" -ForegroundColor Cyan
